...
def product(*, domain="work", sensitivity="internal", review="accepted", targets=None):
...
with tempfile.TemporaryDirectory(prefix="wikified-page-governance-") as temporary:
    ...
    allowed("unrelated JSON cannot override", json_product.replace('"sources": []',
        '"sources": [{"domain": "personal", "review-state": "rejected"}]'))
    checked += 18
    ...
    invalid = {
        ...
        "product review mismatch": valid.replace('"review-state": "accepted"', '"review-state": "pending"'),
        ...
        "AI bypass review": valid.replace('"review-state": "accepted"', '"review-state": "not-required"'),
        "mind object flow": quartet() + '\nmind2one: {version: 99}',
        "JSON duplicate identity": json_product.replace('"id": "memory-phase1"', '"id": "different", "id": "memory-phase1"'),
        "JSON unknown version": json_product.replace('"version": 1', '"version": 2'),
